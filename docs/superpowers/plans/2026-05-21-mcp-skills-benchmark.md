...
**Architecture:** Standalone pnpm workspace (`to-skills-bench/`) mirroring the `to-skills` layout; `@skillit/mcp` pinned as a versioned dep. Four packages: `harness` (runner + result schema), `tasks` (corpus + ground truth), `servers` (pinned MCP servers), `report` (chart generation). Append-only JSONL results committed for diffing; charts derive, never compute.
...
**Prerequisite:** Phase 0 (`to-skills refine`) must be complete and `@skillit/client` published (or available via `workspace:*` link) before running condition D in H3.
...
import type { McpTransport } from '@skillit/mcp';
...
async function measureServer(name: string, version: string): Promise<void> {
  // Lazy import to avoid loading @skillit/mcp at module level
  const { extractMcpSkill, renderSkill } = await import('@skillit/mcp');
...
- [ ] **Step 2: Install @skillit/mcp (versioned dep, not workspace link)**

```bash
pnpm add @skillit/mcp --filter @bench/harness
```

Replace with the published version number, e.g. `@skillit/mcp@0.4.0`. Use workspace link only if running against an unpublished build.
...
async function measureMcpColdStart(name: string): Promise<number[]> {
  const { extractMcpSkill } = await import('@skillit/mcp');
  const serverInfo = SERVERS[name]!;
  const times: number[] = [];
...
// Conditions B/C/D require SKILL.md files generated by @skillit/mcp
