...
export async function generateCliSkill(opts: GenerateSkillOpts): Promise<void> {
  ...
  if (pkgDir) attachDepSkills(skill, pkgDir);
  writeCliSkill(skill, {
    outDir: opts.outDir,
    ...(opts.maxTokens !== undefined ? { maxTokens: opts.maxTokens } : {})
  });
...
function contentTypeFromFile(file: string, outDir: string): SkillitContentType | undefined {
  ...
  if (isSkillitContentType(bucket)) return bucket;
  return undefined;
}

function isSkillitContentType(value: string | undefined): value is SkillitContentType {
  return typeof value === 'string' && SKILLIT_CONTENT_TYPES.includes(value as SkillitContentType);
}
